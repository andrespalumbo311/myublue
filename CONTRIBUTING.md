...
| **Application ID Mismatch** | Linking or configuring services using assumed Application IDs or generic names (e.g., `helium.desktop`) instead of verified Flatpak IDs (e.g., `net.imput.helium.desktop`) causes identity crises in MimeType handlers and Portals. Some apps also hardcode checks for a specific filename (e.g., `helium.desktop`) to verify their own installation. | **Always** align symlinks and MIME integrations with the official ID. If the app requires a legacy name for internal checks, create a shadow `.desktop` file with `NoDisplay=true` to avoid duplicate icons in the launcher. |
...
| **Missing Remote Desktop Backend (wlroots)** | Il controllo remoto (Valent) fallisce su Niri se il portale scelto non esporta l'interfaccia `RemoteDesktop` (come accade con `wlr` in alcune distro) o se mancano i permessi per `/dev/uinput`. | **Usare** `xdg-desktop-portal-hyprland` per un supporto completo a `RemoteDesktop` su wlroots e assicurare i permessi a `/dev/uinput` via `tmpfiles.d`. |
