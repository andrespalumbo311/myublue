...
| **Missing Remote Desktop Backend** | Il controllo remoto (Valent) fallisce su Niri se il pacchetto `gnome-remote-desktop` è inerte, se mancano i permessi Flatpak, le chiavi dconf o se il demone fallisce l'inizializzazione dei certificati. | **Sempre** installare il backend, concedere permessi `RemoteDesktop/ScreenCast` via Flatpak override, abilitare il mirroring in dconf, avviare il demone con `--headless` via systemd user e pre-inizializzare con `grdctl`. |
