...
| **GPU Driver Case Sensitivity** | `LIBVA_DRIVER_NAME` is case-sensitive. Using `ihd` instead of `iHD` for Intel Media Driver causes fallback to software rendering. | **Always** use `iHD` (exact case) for Intel Gen8+ hardware acceleration. |
