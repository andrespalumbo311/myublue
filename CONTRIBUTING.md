...
| **Package Source & Namespace Integrity** | Assuming package availability, naming, or external installer flags without verification. | 1. Verify exact package names. 2. If using external installers (e.g., `install.sh`), **always** check their help output (`--help`) for correct flags (e.g., `--bin-dir` vs `--dest`). 3. If unavailable in repos, use **multi-stage binary injection** in the `builder` stage. |
